...
export const ProjectForm = ({ manageUrl = URL_CREAT_PROJECT, type = RequestTypes.Post, setShowUserInfo }: Props) => {
  ...
  const handleCancel = () => {
    form.resetFields();
    params.id ? navigate(PATHS.PROJECT_OVERVIEW.replace(':id', params.id as string)) : navigate(PATHS.PROJECTS);
    if (setShowUserInfo) {
      setShowUserInfo(false);
    }
  };
